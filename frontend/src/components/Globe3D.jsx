...
import {
  ...
  Zap, Flame, CheckCircle2, Shield, Target, Rocket, Activity, ChevronRight,
  BookOpen, Info, ArrowUpRight, Award, Compass, HelpCircle, Layers, ShieldCheck,
  Disc, Eye, Cpu, Fuel, TrendingDown, Check
} from 'lucide-react';
...
// High-fidelity fallback procedural Earth canvas if network is offline
...
function createUltraBrightSatelliteMesh(busColorHex = 0xffdf00, wingColorHex = 0x0066ff, beaconColorHex = 0x00ffcc) {
  ...
  dish.position.set(0, 0.24, 0.08);
  group.add(dish);

  // 5. Optical Navigation Beacon Strobe
  ...
  // 6. Rear High-Energy Ion Propulsion Thruster Assembly
...
export default function Globe3D({ 
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // 1. Main Three.js Scene Setup with Realistic 4K Lighting & Textures
  // ───────────────────────────────────────────────────────────────────────────
  useEffect(() => {
    ...
    // WebGL Renderer with Tone Mapping
    let renderer;
    try {
    ...
    // NASA Blue Marble Day & Night Earth Textures
    const dayTex = loader.load(NASA_EARTH_DAY, undefined, undefined, () => createProceduralEarthCanvas());
    dayTex.colorSpace = THREE.SRGBColorSpace;
    const nightTex = loader.load(NASA_EARTH_NIGHT);
    ...
    const animate = (time) => {
      ...
      try {
        ...
        if (earthGroupRef.current) {
          if (autoRotate && !isDraggingRef.current && simModeRef.current === 'live') {
  ...
  // ───────────────────────────────────────────────────────────────────────────
  useEffect(() => {
    if (!earthGroupRef.current || !satellitesGroupRef.current) return;
    ...
    if (simMode === 'live') {
      const displaySats = objects && objects.length > 0 ? objects.slice(0, qualityProfile.maxLiveSatellites) : [
        ...
        { norad_id: 48274, name: 'STARLINK-2401', position_km: [6920, -500, 1200] },
        { norad_id: 20580, name: 'HST (HUBBLE)', position_km: [6910, 1500, -800] },
        { norad_id: 43226, name: 'TIANGONG (CSS)', position_km: [6760, -800, 2100] },
      ];

      displaySats.forEach((sat, i) => {
        const pos = sat.position_km || [6500 + i * 20, 0, 0];
        const rKm = Math.sqrt(pos[0] * pos[0] + pos[1] * pos[1] + pos[2] * pos[2]);
        const rNorm = Math.max(earthRadius * 1.05, (rKm / 6378.137) * earthRadius);

        const inc = ((sat.norad_id * 17) % 90) * (Math.PI / 180);
        const raan = ((sat.norad_id * 31) % 360) * (Math.PI / 180);
        const initAngle = ((sat.norad_id * 43) % 360) * (Math.PI / 180);

        const isSpecial = sat.name.includes('ISS') || sat.name.includes('IRIDIUM') || sat.name.includes('COSMOS') || sat.name.includes('TIANGONG') || sat.name.includes('HST');

        const altitudeFactor = Math.pow(6778.0 / Math.max(6500, rKm), 1.5);
        const orbitalSpeed = (0.7 + ((sat.norad_id % 7) * 0.1)) * altitudeFactor;
  ...
  return (
    <div className="flex flex-col gap-6 font-sans">
      ...
      ───────────────────────────────────────────────────────────────────── */}
      <div className="relative w-full h-[580px] lg:h-[660px] rounded-sm overflow-hidden border border-border/80 bg-[#02050e] shadow-2xl select-none font-sans">
        {/* 3D Canvas */}
        <div ref={containerRef} className="w-full h-full cursor-grab active:cursor-grabbing" />

        {/* Top Center Mode Switcher */}
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-1.5 p-1.5 bg-black/80 border border-white/10 rounded-sm shadow-2xl backdrop-blur-md text-xs">
          <button
            onClick={() => handleModeSelect('live')}
            className={`px-3 py-1.5 rounded-xs text-xs font-medium transition-all flex items-center gap-1.5 ${
              simMode === 'live'
        ...
        <div className="absolute top-4 left-4 z-10 flex items-center gap-2 px-3 py-1.5 rounded-sm bg-black/80 border border-white/10 text-xs backdrop-blur-md shadow-lg font-mono">
        ...
        <div className="absolute top-4 right-4 z-20 flex items-center gap-2">
          <button
            ...
            title="Reset Camera"
            className="size-8 rounded-xs bg-black/80 border border-white/10 backdrop-blur-md flex items-center justify-center text-white/70 hover:text-white hover:bg-black/90 transition-all shadow-md"
          >
          ...
          <button
            ...
            className={`size-8 rounded-xs border backdrop-blur-md flex items-center justify-center transition-all shadow-md ${
        ...
        {simMode !== 'live' && (
          <div className="absolute bottom-4 left-4 right-4 z-20 flex flex-col gap-2 p-3 rounded-sm bg-black/85 border border-white/10 backdrop-blur-md shadow-2xl">
            {/* Progress Track */}
      ...
      ───────────────────────────────────────────────────────────────────── */}
      {simMode === 'avoidance_2009' && (
        <div className="p-6 rounded-sm border border-border/80 bg-card/60 backdrop-blur-md shadow-xl space-y-6">
          {/* Station Header */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-border/60">
            <div className="flex items-center gap-3">
              <div className="size-9 rounded-sm bg-primary/10 border border-primary/30 flex items-center justify-center text-primary">
            ...
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-1 rounded-xs bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 text-xs font-mono font-semibold flex items-center gap-1.5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="p-4 rounded-sm bg-card border border-border/60 flex flex-col justify-between">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            {/* 1. RTN Burn Vector */}
            <div className="p-4 rounded-sm bg-card/80 border border-border/60 space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs font-mono">
                <div className="p-2 rounded-xs bg-background border border-border/60 flex flex-col">
                  <span className="text-[10px] text-muted-foreground">Radial (R̂)</span>
                  <span className="font-bold text-foreground mt-1">0.00 m/s</span>
                </div>
                <div className="p-2 rounded-xs bg-primary/10 border border-primary/30 flex flex-col">
            ...
            <div className="p-4 rounded-sm bg-card/80 border border-border/60 space-y-3">
              ...
              <div className="p-3 rounded-xs bg-primary/5 border border-primary/20 font-mono text-xs text-primary space-y-1">
            ...
            <div className="p-4 rounded-sm bg-card/80 border border-border/60 space-y-3">
              ...
              <div className="p-3 rounded-xs bg-emerald-500/5 border border-emerald-500/20 font-mono text-xs text-emerald-400 space-y-1">
          ...
          {/* Astrodynamic Principles & Physics Rationale */}
          <div className="p-4 rounded-sm bg-background border border-border/70 space-y-2.5">
            <div className="flex items-center gap-2 text-primary font-bold text-xs">
              <Shield className="size-4" />
              <span>Key Astrodynamic Principles & Physics Rationale:</span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs text-muted-foreground leading-relaxed">
              <div className="p-3 rounded-xs bg-card border border-border/50">
