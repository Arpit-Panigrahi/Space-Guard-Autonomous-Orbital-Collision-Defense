...
def analytic_pc(miss_vector_km: np.ndarray, sigma_km: float, hbr_km: float) -> float:
    ...
    """
    if sigma_km <= 0 or hbr_km <= 0:
        return 0.0

    d = float(np.linalg.norm(miss_vector_km))
    
    # Exponents
    miss_term = (d**2) / (2.0 * sigma_km**2)
    hbr_term = (hbr_km**2) / (2.0 * sigma_km**2)
    
    # Use -np.expm1(-x) = 1 - exp(-x) for robust precision near zero
    pc = np.exp(-miss_term) * (-np.expm1(-hbr_term))
